...
def run_all(db: TrackerDB) -> int:
    # Exclude Tiny Startups (210 noise / 2 success, 基本失效)
    excluded = {"tinystartups"}
    active_trackers = {k: v for k, v in TRACKERS.items() if k not in excluded}
    print(f"Running {len(active_trackers)} trackers...\n\n")
    
    total = 0
    for name, cls in active_trackers.items():
        print(f"[{name}]")
        tracker = cls()
        items = tracker.run()
        if items:
